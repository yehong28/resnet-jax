...
- Random seed control
  ...
  - Epochs: `torch.utils.data.distributed.DistributedSampler` for distirbuted training. The training curve is exactly the same with or without `train_loader.sampler.set_epoch(epoch)`, and need to rewrite `map(dataloader)` to `collate_fn` which seems decrease the data loading speed, so it is removed.
